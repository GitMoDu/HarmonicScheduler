#ifndef _HARMONIC_SCHEDULER_PROFILING_TIMELINE_SYSTEM_LEVEL_PROFILING_h
#define _HARMONIC_SCHEDULER_PROFILING_TIMELINE_SYSTEM_LEVEL_PROFILING_h
...
namespace Harmonic
{
	namespace Profiling
	{
		namespace Timeline
		{
			...
			class SystemLevelProfiling
				: public ISystemLevelProfiler
			...
			private:
				SystemTimelineSample Traces[TimelineSampleCount]{};
				size_t TraceCount = 0;
			...
			public:
				...
				bool SetTimelineListener(ISystemLevelListener* resultListener) override
				{
			...
			private:
				...
				void AddTrace(const uint32_t timestamp, const task_handle_t handle)
				{
					...
					{
						Traces[TraceCount] = SystemTimelineSample{ timestamp, handle };
						TraceCount++;
					}
				}
	...
	// Alias for the system-level timeline profiling scheduler.
	template<task_index_t MaxTaskCount, bool IdleSleepEnabled, size_t TraceSampleCount>
	using SchedulerTimelineSystemLevel = Profiling::Timeline::SystemLevelProfiling<MaxTaskCount, IdleSleepEnabled, TraceSampleCount>;
}
#endif